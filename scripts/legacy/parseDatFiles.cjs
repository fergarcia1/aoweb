...
const SOURCE_DIR = path.join(__dirname, '../tools/imperium-clasico-ref/Server/Dat');
...
function decodeDat(buf) {
  return buf[1] === 0 ? buf.toString('utf16le') : buf.toString('latin1');
}

// 3. Parse Objs
function parseObjs() {
  const buf = fs.readFileSync(path.join(SOURCE_DIR, 'obj.dat'));
  const content = decodeDat(buf);
  const data = parseIni(content);
